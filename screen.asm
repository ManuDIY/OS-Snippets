; TODO: Create a limited version of os_string_input
; Really can't think of a good name at the moment. :(
; The user will specify the number of character to input in advance.
; There are so many problems with the current one that returns up to 255 bytes
; * Can cause overflows in BASIC strings - These are only 128 bytes.
; * Causes problems for os_input_dialog - This uses os_string_input to get internally
;	> Looks like it would return ~40 bytes but could actually return up to 255 bytes.
;	> Doesn't specify the return length.
;	> Input text "overflows" past the end of the on screen dialog.
; * The kernel and user programs need to assign unnessecarily large buffers.
; * I can't spell unnessecarily.
; * No "editing" supported - Would be good to be able to use left/right keys
;
; A new length limiting version should not be beyond the MikeOS simplicity requirement.
; Old version should be kept (but DEPRECATED) and could be reimplemented as the new version with 255 as parameter
; Might actually *reduce* kernel size by implementing this (smaller buffers).
